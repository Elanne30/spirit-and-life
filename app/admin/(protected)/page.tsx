...
export default async function AdminDashboardPage() {
  const [newsletterSummary, pushSummary, reflectionItems, journalItems, bookItems, articles] = await Promise.all([
  ...
  const contentStats: Array<{ title: string; description: string; icon: typeof FileText; href: string; count: number; secondary: string }> = [
    { title: "Articles", description: "Philosophy, apologetics, theology, and hard questions", icon: PenLine, href: "/admin/content/article", count: articles.length, secondary: `${articles.length} published` },
  ...
  return (
    <section className="admin-dashboard">
      <div className="admin-page-introduction">
        <div><p className="eyebrow">Overview</p><h2>Welcome back</h2><p>Here&apos;s what&apos;s happening across Spirit &amp; Life Eternal.</p></div>
        <span className="admin-date"><CalendarDays aria-hidden="true" size={15} />{today}</span>
      </div>

      <div className="admin-grid">
        {contentStats.map((card) => { const Icon = card.icon; return <article className="admin-card admin-summary-card" key={card.title}><div className="admin-card-heading"><span className="admin-icon"><Icon aria-hidden="true" size={18} strokeWidth={1.8} /></span><h3>{card.title}</h3></div><p>{card.description}</p><div className="admin-card-detail admin-card-metrics"><span><strong>{card.count}</strong><small>Total</small></span><span><strong>{card.secondary.split(" ")[0]}</strong><small>{card.secondary.substring(card.secondary.indexOf(" ") + 1)}</small></span></div><Link className="admin-card-link" href={card.href}>Open workspace <ArrowRight aria-hidden="true" size={14} /></Link></article>; })}
      </div>

      <div className="admin-dashboard-lower">
        <article className="admin-card admin-overview-panel"><div className="admin-panel-heading"><div><h3><BarChart3 aria-hidden="true" size={17} />Recent Content</h3><p>Latest records from the Reflection, Journal, and Book libraries.</p></div><Link className="admin-outline-link" href="/admin/content">View all <ArrowRight size={13} /></Link></div><ul className="admin-content-list">{recentContent.map((item) => <li key={`${item.contentType}-${item.slug}`}><Link href={`/admin/content/${item.contentType}/${item.slug}`}><strong>{item.title}</strong><small>{item.contentType} · {statusLabel(item)}{item.updatedAt ? ` · ${item.updatedAt}` : item.date ? ` · ${item.date}` : ""}</small></Link></li>)}{!recentContent.length ? <li><span className="quiet-note">No content records are available yet.</span></li> : null}</ul></article>
        <article className="admin-card admin-overview-panel"><div className="admin-panel-heading"><div><h3>Published Articles</h3><p>Articles are managed separately from the existing content library.</p></div><Link className="admin-outline-link" href="/admin/content/article">View Articles <ArrowRight size={13} /></Link></div><ul className="admin-content-list">{articles.slice(0, 8).map((article) => <li key={article.id}><Link href={`/admin/content/article/${article.slug}`}><strong>{article.title}</strong><small>article · Published</small></Link></li>)}{!articles.length ? <li><span className="quiet-note">No Articles have been published yet.</span></li> : null}</ul></article>
      </div>

      <article className="admin-card" style={{ marginTop: "0.8rem" }}><div className="admin-panel-heading"><div><h3>Quick actions</h3><p>Start common Admin tasks using the existing routes.</p></div></div><div className="admin-content-overview-grid">{[
        { label: "New Article", href: "/admin/content/article/new", icon: PenLine },
      ...
      <div className="admin-grid" style={{ marginTop: "0.8rem", gridTemplateColumns: "repeat(auto-fit, minmax(14rem, 1fr))" }}>{operations.map((card) => { const Icon = card.icon; return <Link className="admin-card admin-summary-card" href={card.href} key={card.title} style={{ textDecoration: "none" }}><div className="admin-card-heading"><span className="admin-icon"><Icon size={18} /></span><h3>{card.title}</h3></div><p>{card.description}</p><div className="admin-card-detail"><strong>{card.detail}</strong></div></Link>; })}</div>
    </section>
  );
}
