    ...
    <div className="admin-stack">
      <form className="admin-form" action={formAction}>
        ...
        <fieldset className="admin-recipient-picker">
          ...
          <div className="admin-recipient-list">{visibleSubscribers.map((subscriber) => <label className="admin-recipient-row" key={subscriber.id}><span className="admin-recipient-details"><strong>{getSubscriberDisplayName(subscriber)}</strong><small>{subscriber.id}</small><small>{subscriber.email}</small></span><input type="checkbox" name="recipientId" value={subscriber.id} checked={selectedIds.includes(subscriber.id)} onChange={(event) => setSelectedIds((current) => event.target.checked ? [...current, subscriber.id] : current.filter((id) => id !== subscriber.id))} /></label>)}{!visibleSubscribers.length ? <p className="quiet-note">No active recipients match this search.</p> : null}</div>
