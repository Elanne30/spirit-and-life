...
export default async function AdminSubscribersPage() {
  ...
  return (
    <section className="admin-management-page">
      ...
      <article className="admin-card admin-subscriber-table-card"><div className="admin-panel-heading"><div><h3><Users size={17} />Subscriber records</h3><p>Stored newsletter subscribers and push subscriptions.</p></div></div><div className="admin-subscriber-table" role="table" aria-label="Subscriber records"><div role="row" className="admin-subscriber-row admin-subscriber-heading"><span>Subscriber ID</span><span>Email / endpoint</span><span>Status</span><span>Subscription type</span><span>Date joined</span><span>Actions</span></div>{newsletterRecords.map((record) => <div role="row" className="admin-subscriber-row" key={`email-${record.id}`}><span title={record.id}>{record.id}</span><span title={record.email}>{record.email}</span><span>{record.status}</span><span>Newsletter</span><span>{new Date(record.created_at).toLocaleDateString()}</span><span>—</span></div>)}{pushRecords.map((record) => <div role="row" className="admin-subscriber-row" key={`push-${record.id}`}><span title={record.id}>{record.id}</span><span title={record.endpoint}>{record.endpoint}</span><span>{record.status}</span><span>Push</span><span>{new Date(record.created_at).toLocaleDateString()}</span><span>—</span></div>)}{!newsletterRecords.length && !pushRecords.length ? <p className="admin-empty-table">No subscriber records are stored yet.</p> : null}</div></article>
    </section>
  );
}
