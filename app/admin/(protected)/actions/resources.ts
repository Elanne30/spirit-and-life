...
import { createDownloadableResourceRecord } from "@/app/lib/resource-repository";
...
export async function createDownloadableResource(input: { title: string; description: string; kind: "PDF" | "Audio" | "Document"; fileUrl: string; fileName?: string; publishedAt?: string }) {
  ...
  try {
    ...
    return { ok: false as const, error: error instanceof Error ? error.message : "Resource could not be saved." };
  }
}
