...
function csv(value: FormDataEntryValue | null) {
  return String(value ?? "").split(",").map((item) => normalizeDraftSlug(item.trim())).filter(Boolean);
}
...
export async function updatePodcastEpisodeAction(formData: FormData) {
  ...
  await updatePodcastEpisode(slug, {
    ...
    topicSlugs,
    seriesSlug,
    questionSlugs: csv(formData.get("questionSlugs")),
    articleSlugs: csv(formData.get("articleSlugs")),
    resourceSlugs: csv(formData.get("resourceSlugs")),
  });
...
export async function savePodcastEpisodeAction(formData: FormData) {
  return updatePodcastEpisodeAction(formData);
}
