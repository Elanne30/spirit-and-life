...
export async function updatePodcastEpisodeAction(formData: FormData) {
  ...
  const slug = String(formData.get("slug") ?? "").trim(); if (!slug) return { ok: false as const, error: "Missing episode." };
  ...
  await updatePodcastEpisode(slug, { title: String(formData.get("title") ?? "").trim(), description: String(formData.get("description") ?? "").trim(), publishedAt: String(formData.get("publishedAt") ?? "").trim(), duration: String(formData.get("duration") ?? "").trim() || null, coverImage: String(formData.get("coverImage") ?? "").trim() || null, audioUrl: String(formData.get("audioUrl") ?? "").trim() || null, transcript: String(formData.get("transcript") ?? "").trim() || null, youtubeUrl: String(formData.get("youtubeUrl") ?? "").trim() || null, topicSlugs, seriesSlug, questionSlugs: csv(formData.get("questionSlugs")), articleSlugs: csv(formData.get("articleSlugs")), resourceSlugs: csv(formData.get("resourceSlugs")) });
  revalidatePath("/admin/podcast"); revalidatePath(`/admin/podcast/${slug}`); revalidatePath("/podcast"); revalidatePath(`/podcast/${slug}`); revalidatePath("/search"); return { ok: true as const };
}
