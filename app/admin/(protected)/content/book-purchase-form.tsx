...
export function BookPurchaseForm({ draftId, initialStatus = "Coming Soon", initialOptions = [], initialPaperbackStatus = "Available Soon", initialPaperbackUrl = "" }: { draftId: string; initialStatus?: "Available" | "Coming Soon"; initialOptions?: PurchaseOption[]; initialPaperbackStatus?: "Available Soon" | "Available"; initialPaperbackUrl?: string }) {
  ...
  return <form action={formAction} className="admin-form">
    <input type="hidden" name="draftId" value={draftId} /><input type="hidden" name="status" value={status} readOnly /><input type="hidden" name="purchaseOptions" value={JSON.stringify(options)} readOnly /><input type="hidden" name="paperbackStatus" value={paperbackStatus} readOnly /><input type="hidden" name="paperbackUrl" value={paperbackUrl} readOnly />
    <div className="admin-section-heading"><p className="eyebrow">Book availability</p><h2>Availability</h2><p>Choose whether this book is ready for readers or still being prepared.</p></div>
    <label>Book status</label><select value={status} onChange={(event) => setStatus(event.target.value as "Available" | "Coming Soon")}><option value="Available">Available</option><option value="Coming Soon">Coming Soon</option></select>
