...
export function ArticleForm({ draft }: Props) {
  ...
  return <>
    {draft ? <section className="admin-article-image-panel" aria-labelledby="article-image-heading">
      <div>
        <p className="admin-form-label" id="article-image-heading">Article image</p>
        <p className="form-note">Upload an image to use on the Article card and at the top of the published Article. JPG, PNG, WebP, or GIF up to 8 MB.</p>
      </div>
      {imagePreview ? <img className="admin-article-image-preview" src={imagePreview} alt="Current Article image" /> : <div className="admin-article-image-empty">No Article image selected yet.</div>}
      <form action={uploadAction} className="admin-article-image-upload">
        <input type="hidden" name="draftId" value={draft.id} />
        <label className="button button-secondary" htmlFor="article-image-upload">{imagePreview ? "Change image" : "Choose image"}</label>
        <input id="article-image-upload" name="image" type="file" accept="image/jpeg,image/png,image/webp,image/gif" onChange={(event) => { const file = event.target.files?.[0]; if (file) setImagePreview(URL.createObjectURL(file)); }} />
        <button className="button button-primary" type="submit" disabled={uploading}>{uploading ? "Uploading..." : "Upload and save image"}</button>
      </form>
    ...
    <form action={formAction} className="admin-form">
      ...
      <label htmlFor="article-image">Article image reference</label>
      <input id="article-image" name="imageReference" defaultValue={draft?.image_reference ?? bodyString(draft, "image")} placeholder="/images/articles/my-article.jpg or https://..." />
      <p className="form-note">Optional. Uploaded images are stored automatically; use this field only when you want to reference an existing image path or approved image URL.</p>
