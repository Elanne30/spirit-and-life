...
export function DraftForm({ initialContentType = "reflection" }: DraftFormProps) {
  ...
  return (
    <form className="admin-form admin-draft-form" action={formAction} style={{ gridTemplateColumns: "minmax(0, 1fr)" }}>
      ...
      <div className="admin-draft-column" style={{ gridColumn: "1 / -1", width: "100%" }}>
        ...
        {(isReflection || isJournal) ? (
          <>
            ...
            <textarea id="draft-introduction" name="introduction" rows={6} placeholder={isReflection ? "Write the introduction to your reflection..." : "Write a short introduction to your journal entry..."} />
          </>
        ) : null}
      ...
      {state.message ? (
        <p className={state.status === "error" ? "form-error" : "form-note"} role="status" style={{ gridColumn: "1 / -1" }}>
          {state.message}
        </p>
      ) : null}
    </form>
