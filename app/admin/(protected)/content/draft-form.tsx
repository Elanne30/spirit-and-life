    ...
    <form className="admin-form admin-draft-form" action={formAction} style={{ gridTemplateColumns: "minmax(0, 1fr)" }}>
      ...
      <div className="admin-draft-column" style={{ gridColumn: "1 / -1", width: "100%" }}>
        ...
        {isReflection || isJournal ? <>
          ...
          <label htmlFor="draft-subtitle">Subtitle</label><input id="draft-subtitle" name="subtitle" type="text" placeholder="Book subtitle" />
          <label htmlFor="draft-author">Author</label><input id="draft-author" name="author" type="text" placeholder="Author name" />
          <label htmlFor="draft-publisher">Publisher</label><input id="draft-publisher" name="publisher" type="text" placeholder="Publisher" />
          <label htmlFor="draft-expected-publication">Expected publication</label><input id="draft-expected-publication" name="expectedPublication" type="text" placeholder="2026" />
          <label htmlFor="draft-length">Length</label><input id="draft-length" name="length" type="text" placeholder="320 pages" />
          <label htmlFor="draft-table-of-contents">Table of contents</label><textarea id="draft-table-of-contents" name="tableOfContents" rows={8} placeholder="Chapter 1\nChapter 2\nChapter 3" />
        </>}
        ...
        <label htmlFor="draft-tags">Tags</label><input id="draft-tags" name="tags" type="text" placeholder="Scripture, Faith, Interpretation" />
        {(isReflection || isJournal) && <><label htmlFor="draft-introduction">Introduction</label><textarea id="draft-introduction" name="introduction" rows={6} placeholder={isReflection ? "Write the introduction to your reflection..." : "Write a short introduction to your journal entry..."} /></>}
      ...
      {(isReflection || isJournal) && <div className="admin-draft-column admin-structure-column" style={{ gridColumn: "1 / -1", width: "100%", maxWidth: "none" }}><h3>Content Structure</h3><ReflectionBodyEditor /></div>}
