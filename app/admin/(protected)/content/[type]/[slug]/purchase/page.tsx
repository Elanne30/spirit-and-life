...
export default async function BookPurchaseSettingsPage({ params }: { params: Promise<{ type: string; slug: string }> }) {
  ...
  const status = body.status === "Available" ? "Available" : "Coming Soon";
  ...
  return <section className="admin-editor-page" style={{ width: "100%", maxWidth: "none", paddingBlock: "1rem 4rem" }}>
    ...
    <article className="admin-editor-card" style={{ width: "100%", maxWidth: "none", padding: "clamp(1rem, 2vw, 2rem)", border: "1px solid var(--line)", background: "var(--surface)", boxShadow: "0 0.45rem 1.25rem var(--shadow)" }}><BookPurchaseForm draftId={draft.id} initialStatus={status} initialOptions={options} initialPaperbackStatus={paperbackStatus} initialPaperbackUrl={paperbackUrl} /></article>
  </section>;
}
