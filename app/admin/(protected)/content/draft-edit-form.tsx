...
const initialActionState: ContentDraftActionState = { status: "idle", message: "" };
...
export function DraftEditForm({ draft }: { draft: ContentDraft }) {
  const [state, formAction, isPending] = useActionState(updateDraftActionSafe, initialActionState);
  ...
  return (<form className="admin-form" action={formAction}>
    ...
    <label htmlFor="edit-content-type">Content type</label><select id="edit-content-type" name="contentType" defaultValue={draft.content_type} required><option value="reflection">Reflection</option><option value="journal">Journal</option><option value="book">Book</option></select>
    ...
    <label htmlFor="edit-date">Date</label><input id="edit-date" name="date" type="text" defaultValue={date} />
    <label htmlFor="edit-reading-time">Reading time</label><input id="edit-reading-time" name="readingTime" type="text" defaultValue={readingTime} />
    <label htmlFor="edit-category">Category</label><select id="edit-category" name="category" defaultValue={draft.category ?? "Biblical Studies"}><option value="Biblical Studies">Biblical Studies</option><option value="Theology">Theology</option><option value="Christian Living">Christian Living</option><option value="Faith & Life">Faith & Life</option><option value="Philosophy">Philosophy</option><option value="Apologetics">Apologetics</option><option value="Church History">Church History</option><option value="SCRIPTURE">SCRIPTURE</option></select>
    <label htmlFor="edit-tags">Tags</label><input id="edit-tags" name="tags" type="text" defaultValue={draft.tags.join(", ")} />
    <label htmlFor="edit-scripture">Scripture</label><input id="edit-scripture" name="scripture" type="text" defaultValue={scripture} />
    <label htmlFor="edit-introduction">Introduction</label><textarea id="edit-introduction" name="introduction" rows={6} defaultValue={draft.introduction ?? ""} />
    <label className="admin-checkbox" htmlFor="edit-featured"><input id="edit-featured" name="featured" type="checkbox" value="yes" defaultChecked={featured} />Feature this reflection</label>
    {draft.content_type === "reflection" || draft.content_type === "journal" ? <ReflectionBodyEditor initialSections={getInitialSections(draft)} initialRichText={getBodyValue(draft.body, "richText")} /> : null}
