...
import { AlignCenter, AlignJustify, AlignLeft, AlignRight, Bold, ChevronDown, IndentDecrease, IndentIncrease, Italic, Link2, List, ListOrdered, Minus, Pilcrow, Plus, Redo2, RotateCcw, Underline } from "lucide-react";
...
const headingOptions: Array<{ label: string; value: HeadingLevel | "paragraph" }> = [
  { label: "Normal", value: "paragraph" }, { label: "Heading 1", value: 1 }, { label: "Heading 2", value: 2 }, { label: "Heading 3", value: 3 },
  { label: "Heading 4", value: 4 }, { label: "Heading 5", value: 5 }, { label: "Heading 6", value: 6 }, { label: "Paragraph", value: "paragraph" },
];
const spacingOptions = [
  { label: "Single", value: 1 }, { label: "1.15", value: 1.15 }, { label: "1.5", value: 1.5 }, { label: "Double", value: 2 },
];
...
export function AdminRichTextEditor({ initialValue, onChange, labelledBy }: AdminRichTextEditorProps) {
  ...
  const currentHeading = editor.isActive("heading") ? `Heading ${Number(attrs.level ?? 1)}` : "Normal";
  ...
  return <div className={styles.editorShell}>
    <div className={styles.toolbar} role="toolbar" aria-label="Rich text formatting">
      ...
      <ToolbarGroup><label className={styles.srOnly} htmlFor="rich-text-font-family">Font family</label><select id="rich-text-font-family" aria-label="Font family" value={currentFontFamily ?? ""} onChange={event => event.target.value ? editor.chain().focus().setFontFamily(event.target.value).run() : editor.chain().focus().unsetFontFamily().run()} className={styles.select}><option value="">Calibri (Body)</option><option value={fontFamilies[0]}>Serif</option><option value={fontFamilies[1]}>Sans serif</option><option value={fontFamilies[2]}>Georgia</option></select><label className={styles.srOnly} htmlFor="rich-text-font-size">Font size</label><select id="rich-text-font-size" aria-label="Font size" value={currentFontSize ?? ""} onChange={event => event.target.value ? editor.chain().focus().setFontSize(event.target.value).run() : editor.chain().focus().unsetFontSize().run()} className={`${styles.select} ${styles.sizeSelect}`}><option value="">{currentFontSizeLabel}</option>{numericFontSizes.map(option => <option key={option.value} value={option.value}>{option.label}</option>)}{fontSizes.filter(size => !numericFontSizes.some(option => option.value === size)).map(size => <option key={size} value={size}>{size.replace("rem", "")}</option>)}</select></ToolbarGroup>
      <ToolbarGroup><Menu name="heading" label="Heading style" value={currentHeading} open={openMenu === "heading"} onToggle={() => setOpenMenu(openMenu === "heading" ? null : "heading")}>
        {headingOptions.map((option, index) => <MenuItem key={`${option.label}-${index}`} label={option.label} active={option.value === "paragraph" ? !editor.isActive("heading") : editor.isActive("heading", { level: option.value })} onClick={() => setHeading(option.value)}><span className={`${styles.headingOption} ${option.value === "paragraph" ? styles.paragraphOption : styles[`heading${option.value}`]}`}>{option.label}</span></MenuItem>)}
      </Menu></ToolbarGroup>
