      ...
      attributes: {
        indent: {
          ...
          renderHTML: (attributes) => {
            ...
            return value ? { "data-indent": value } : {};
          },
        },
        firstLineIndent: {
          default: 0,
          ...
          renderHTML: (attributes) => {
            ...
            return value ? { "data-first-line-indent": value } : {};
          },
        },
        rightIndent: {
          default: 0,
          ...
          renderHTML: (attributes) => {
            ...
            return value ? { "data-right-indent": value } : {};
          },
        },
        lineSpacing: {
          default: 1,
          ...
          renderHTML: (attributes) => {
            const value = Number(attributes.lineSpacing ?? 1);
            return value !== 1 ? { "data-line-spacing": value } : {};
          },
        },
      },
    }];
  ...
  return (
    <div className={`${styles.dropdown} ${className}`}>
      <button
        ...
        title={label}
      >
        <span className={styles.dropdownValue}>{value || label}</span>
        <ChevronDown size={14} />
      </button>
...
function RulerMarker({
  ...
  return (
    <button
      ...
      aria-label={label}
      title={label}
      className={`${styles.rulerMarker} ${styles[`rulerMarker${type}`]}`}
      style={{ left: `${left}%` }}
      onPointerDown={(event) => {
      ...
      onPointerMove={(event) => {
        if (dragging.current) onChange(event.clientX);
      }}
...
export function AdminRichTextEditor({ initialValue, onChange, labelledBy }: AdminRichTextEditorProps) {
  ...
  const editor = useEditor({
    ...
    extensions: [
      StarterKit.configure({
        ...
        link: { openOnClick: false, autolink: false, linkOnPaste: true, protocols: ["http", "https", "mailto"] },
      }),
      UnderlineMark,
    ...
    onUpdate: ({ editor: instance }) => onChange({
      format: "spirit-and-life-rich-text",
      version: 1,
      content: instance.getJSON() as RichTextDocument["content"],
    }),
  });
  ...
  useEffect(() => {
    ...
    const close = (event: MouseEvent) => {
      if (!(event.target instanceof HTMLElement) || !event.target.closest(`.${styles.dropdown}`)) setOpenMenu(null);
    };
  ...
  const blockAttrs = editor.getAttributes(blockType) as {
    level?: number;
    indent?: number;
  ...
  const currentLevel = editor.isActive("heading") ? Number(blockAttrs.level ?? 1) : "paragraph";
  const currentHeadingLabel = editor.isActive("heading") ? `Heading ${currentLevel}` : "Normal";
  ...
  const setRulerPosition = (kind: "first" | "body" | "right", clientX: number) => {
    ...
    const inches = (percent / 100) * PAGE_WIDTH_IN;
    if (kind === "right") {
      const value = Math.round(Math.max(0, Math.min(MAX_INDENT, (PAGE_WIDTH_IN - DEFAULT_MARGIN_IN - inches) / INDENT_STEP_IN)));
      setBlockAttribute("rightIndent", value);
      return;
    }
    const value = Math.round(Math.max(0, Math.min(MAX_INDENT, (inches - DEFAULT_MARGIN_IN) / INDENT_STEP_IN)));
    setBlockAttribute(kind === "first" ? "firstLineIndent" : "indent", value);
  };
    ...
    <div className={styles.editorShell}>
      <div className={styles.toolbar} role="toolbar" aria-label="Rich text formatting">
        ...
        <ToolbarGroup>
          ...
          >
            <option value="">{currentFontSizeLabel || "11"}</option>
            {numericFontSizes.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}
            {fontSizes.filter((size) => !numericFontSizes.some((option) => option.value === size)).map((size) => <option key={size} value={size}>{size.replace("rem", "")}</option>)}
          </select>
        </ToolbarGroup>
        ...
        <ToolbarGroup>
          <Dropdown label="Paragraph" value={<Pilcrow size={18} />} open={openMenu === "paragraph"} onToggle={() => setOpenMenu(openMenu === "paragraph" ? null : "paragraph")}>
            ...
            <DropdownItem label="Double line spacing" active={currentLineSpacing === 2} onClick={() => { setBlockAttribute("lineSpacing", 2); setOpenMenu(null); }}>Double line spacing</DropdownItem>
          </Dropdown>
        </ToolbarGroup>
        ...
        <div className={styles.canvasRow}>
          ...
          <div className={styles.pageColumn}>
            <div ref={rulerRef} className={styles.horizontalRuler} aria-label="Document ruler">
              ...
              <RulerMarker type="right" label="Right indent" left={rulerRight(currentRightIndent)} onChange={(clientX) => setRulerPosition("right", clientX)} />
            </div>

            <div className={styles.pageScroller}>
              <div className={styles.page} style={documentZoomStyle}>
      ...
      <div className={styles.statusBar}>
        ...
        <div className={styles.zoomControl}>
          <button type="button" aria-label="Zoom out" title="Zoom out" onClick={() => setZoom((value) => Math.max(75, value - 5))}>−</button>
          <input aria-label="Zoom" type="range" min="75" max="150" step="5" value={zoom} onChange={(event) => setZoom(Number(event.target.value))} />
          <button type="button" aria-label="Zoom in" title="Zoom in" onClick={() => setZoom((value) => Math.min(150, value + 5))}>+</button>
          <span>{zoom}%</span>
        </div>
      </div>
    </div>
