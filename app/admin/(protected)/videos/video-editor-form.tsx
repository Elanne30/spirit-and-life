...
function slugify(value: string) {
  return value.normalize("NFKD").replace(/[\u0300-\u036f]/g, "").toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "").replace(/-+/g, "-").slice(0, 120);
}
...
export function VideoEditorForm({ video }: { video?: VideoRecord }) {
  ...
  async function save(event: React.FormEvent<HTMLFormElement>) {
    ...
    form.set("thumbnailUrl", thumbnailUrl);
    ...
    <form className="admin-form video-editor-form" onSubmit={save}>
      ...
      <div className="video-editor-grid">
        <div>
          ...
          <select id="video-status" name="status" defaultValue={video?.status ?? "draft"}>
        ...
        <div>
          <label htmlFor="video-published-at">Publication date</label>
          <input id="video-published-at" name="publishedAt" type="datetime-local" defaultValue={video?.publishedAt ? new Date(video.publishedAt).toISOString().slice(0, 16) : ""} />
        </div>
      </div>
      ...
      <fieldset className="video-destinations">
        ...
        <div className="video-destination-grid">
          {VIDEO_DESTINATIONS.map((destination) => <label className="admin-checkbox" key={destination}><input type="checkbox" name="destination" value={destination} defaultChecked={video?.destinations.includes(destination)} />{destinationLabels[destination]}</label>)}
        </div>
      </fieldset>
      ...
      <div className="video-editor-actions">
        <button className="button button-primary" type="submit" disabled={saving}>{saving ? "Saving…" : video ? "Save changes" : "Save video"}</button>
