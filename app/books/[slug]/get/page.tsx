...
function StoreMark({ store }: { store: string }) {
  ...
  if (normalized.includes("selar")) return <span className="store-wordmark selar-mark"><img className="selar-logo" src="https://selar.com/favicon.ico" alt="" aria-hidden="true" />selar</span>;
...
export default async function GetBookPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <main className="book-get-page">
      ...
      <style>{`
        .book-get-page{min-height:100vh;background:var(--background);color:var(--foreground)}
        ...
        .book-store-grid{display:grid;gap:1rem}.book-store-card{display:grid;grid-template-columns:6rem 1fr auto;align-items:center;gap:1.2rem;padding:1.15rem 1.25rem;border:1px solid var(--line);background:var(--surface);color:var(--foreground);text-decoration:none;box-shadow:0 .7rem 2rem var(--shadow);transition:transform 180ms ease,border-color 180ms ease}.book-store-card:hover{transform:translateY(-2px);border-color:color-mix(in srgb,var(--accent) 55%,var(--line))}.book-store-brand{display:grid;place-items:center;min-height:3rem}.store-wordmark{display:inline-flex;align-items:center;gap:.5rem;font-weight:800;font-size:1.25rem;letter-spacing:-.04em}.store-wordmark img.selar-logo{width:2rem;height:2rem;object-fit:contain;border-radius:.35rem}.amazon-mark{font-family:Arial,sans-serif}.amazon-smile{display:block;font-size:1.25rem;line-height:.25;margin-left:2rem}.selar-mark{font-weight:900}.store-initial{display:grid;place-items:center;width:2.8rem;height:2.8rem;border-radius:.7rem;background:var(--surface-muted);font-weight:800;color:var(--accent)}.book-store-copy{display:grid;gap:.2rem}.book-store-copy strong{font-size:1.05rem}.book-store-copy small{color:var(--muted)}.book-store-action{display:inline-flex;align-items:center;gap:.55rem;padding:.85rem 1rem;background:var(--accent);color:#18130a;font-weight:700;white-space:nowrap}.book-no-stores{display:grid;gap:.35rem;padding:1.5rem;border:1px dashed var(--line);background:var(--surface);color:var(--muted)}.book-no-stores strong{color:var(--foreground)}
        .paperback-card{display:flex;align-items:center;justify-content:space-between;gap:2rem;margin-top:3rem;padding:1.5rem;border:1px solid var(--line);background:var(--surface);box-shadow:0 .7rem 2rem var(--shadow)}.paperback-card h2{margin:.25rem 0 .45rem;font-size:1.4rem}.paperback-card p:not(.eyebrow){margin:0;color:var(--muted);line-height:1.6}.paperback-status{display:inline-flex;align-items:center;gap:.45rem;white-space:nowrap;padding:.55rem .8rem;border:1px solid var(--line);border-radius:999px;color:var(--muted);font-size:.8rem;font-weight:700}.book-get-trust{display:grid;grid-template-columns:repeat(3,1fr);margin-top:1rem;border:1px solid var(--line);background:var(--surface);border-radius:.5rem;overflow:hidden}.book-get-trust>div{display:flex;align-items:center;gap:.8rem;padding:1.25rem;border-right:1px solid var(--line);color:var(--accent)}.book-get-trust>div:last-child{border-right:0}.book-get-trust span{display:grid;gap:.25rem}.book-get-trust strong{font-size:.9rem;color:var(--foreground)}.book-get-trust small{font-size:.78rem;line-height:1.4;color:var(--muted)}
        @media(max-width:760px){.book-get-hero-inner{grid-template-columns:1fr;min-height:auto;padding:3.5rem 0}.book-get-cover{order:-1;justify-content:flex-end}.book-get-cover img{width:min(58%,15rem);max-height:22rem}.book-get-hero-copy{margin-top:-1rem}.book-hero-features{flex-wrap:wrap}.book-get-back{margin-bottom:2rem}.book-store-card{grid-template-columns:4rem 1fr}.book-store-action{grid-column:2;justify-self:start}.book-get-trust{grid-template-columns:1fr}.book-get-trust>div{border-right:0;border-bottom:1px solid var(--line)}.book-get-trust>div:last-child{border-bottom:0}.paperback-card{align-items:flex-start;flex-direction:column}}
      `}</style>
    </main>
  );
}
