  ...
  return (
    <main>
      ...
      <section className="mx-auto max-w-6xl px-6 py-14 sm:px-8 sm:py-18">
        <a href="/series" className="text-xs font-semibold uppercase tracking-[0.14em] text-muted-foreground hover:text-foreground">← All series</a>
        <div className="mt-8 grid gap-6 lg:grid-cols-[0.8fr_1.2fr]">
          <section className="rounded-2xl border bg-[color:var(--surface)] p-6 sm:p-8">
            ...
            <div className="mt-6 flex flex-wrap gap-2">
              {discovery.topics.map((topic) => <a key={topic.slug} href={`/topics/${topic.slug}`} className="rounded-full border px-3 py-2 text-sm transition hover:border-[color:var(--accent)]">{topic.name}</a>)}
            </div>
          </section>
          <section className="rounded-2xl border bg-[color:var(--surface)] p-6 sm:p-8">
            ...
            <div className="mt-5 space-y-3">{discovery.questions.length ? discovery.questions.map((question) => <a key={question.slug} href={`/questions/${question.slug}`} className="block rounded-xl border p-5 transition hover:-translate-y-0.5 hover:border-[color:var(--accent)]"><span className="text-xs uppercase tracking-[0.12em] text-muted-foreground">Question</span><h3 className="mt-2 font-serif text-xl">{question.question}</h3><span className="mt-3 inline-block text-xs font-semibold uppercase tracking-[0.12em] text-[color:var(--accent)]">Open question ↗</span></a>) : <p className="text-sm text-muted-foreground">Questions will appear here as they are added.</p>}</div>
          </section>
        </div>
        <section className="mt-6 rounded-2xl border bg-[color:var(--surface)] p-6 sm:p-8">
          <div className="flex items-center justify-between gap-4 border-b pb-4"><div><p className="text-xs font-semibold uppercase tracking-[0.14em] text-[color:var(--accent)]">Write-ups</p><h2 className="mt-1 font-serif text-3xl">Articles in this series</h2></div><span className="text-xs text-muted-foreground">{discovery.articles.length}</span></div>
          <div className="mt-5 space-y-3">
            {discovery.articles.length ? discovery.articles.map((article, index) => (
              <a key={article.id} href={`/articles/${article.slug}`} className="group flex gap-4 rounded-xl border p-5 transition hover:-translate-y-0.5 hover:border-[color:var(--accent)] hover:shadow-sm">
                <span className="pt-1 text-sm font-semibold text-muted-foreground">{String(index + 1).padStart(2, "0")}</span>
                <span className="min-w-0">
                  ...
                  {article.introduction ? <p className="mt-2 text-sm text-muted-foreground line-clamp-2">{article.introduction}</p> : null}
                  <span className="mt-3 inline-block text-xs font-semibold uppercase tracking-[0.12em] text-[color:var(--accent)]">Open article ↗</span>
                </span>
              </a>
            )) : <p className="text-sm text-muted-foreground">Published articles assigned to this series will appear here automatically.</p>}
          </div>
        </section>
      </section>
    </main>
