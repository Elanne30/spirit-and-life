...
export default function SeriesPage() {
  ...
  return (
    <main className="bg-[color:var(--background)] text-[color:var(--foreground)] transition-colors duration-200" style={{ backgroundColor: "var(--background)", color: "var(--foreground)" }}>
      ...
      />
      <section className="mx-auto max-w-6xl px-6 py-14 sm:px-8 sm:py-16" aria-labelledby="series-heading" style={{ backgroundColor: "var(--background)", color: "var(--foreground)" }}>
        ...
        <div className="grid gap-5 sm:grid-cols-2">
          {series.map((item, index) => (
            <Link key={item.slug} href={`/series/${item.slug}`} className="group overflow-hidden rounded-xl border shadow-sm transition duration-200 hover:-translate-y-1 hover:shadow-md" style={{ backgroundColor: "var(--surface)", borderColor: "var(--line)", color: "var(--foreground)" }}>
              <div className="relative aspect-[16/8] overflow-hidden" style={{ backgroundColor: "var(--surface-muted)" }}>
                ...
                <span className="absolute bottom-3 left-3 rounded-full px-3 py-1 text-xs font-semibold backdrop-blur-sm" style={{ backgroundColor: "color-mix(in srgb, var(--surface) 90%, transparent)", color: "var(--foreground)" }}>{String(index + 1).padStart(2, "0")}</span>
              </div>
