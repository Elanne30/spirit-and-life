...
async function buildSearchIndex(): Promise<IndexedSearchResult[]> {
  ...
  return [
    ...
    ...PODCAST_EPISODES.map((episode): IndexedSearchResult => ({ type: "Podcast", title: episode.title, description: episode.description, href: `/podcast/${episode.slug}`, meta: [episode.publishedAt, episode.duration].filter(Boolean).join(" · "), searchText: ["Podcast", episode.title, episode.description, episode.publishedAt, episode.duration ?? "", ...episode.topicSlugs, episode.seriesSlug ?? "", ...(episode.questionSlugs ?? []), episode.transcript ?? ""].join(" ") })),
