...
function addDuplicateErrors(values: string[], label: string, errors: string[]) {
  ...
  for (const duplicate of duplicates) errors.push(`${label} has duplicate value "${duplicate}".`);
}
...
function addRelationshipErrors(
  ...
  for (const { key, values, valid } of relationChecks) {
    ...
    for (const value of values) {
      if (key === "relatedBookSlugs" && deletedBookSlugs.has(value)) continue;
      if (!valid.has(value)) errors.push(`${label} "${slug}" references missing ${key} value "${value}".`);
    }
  }
}

export async function ensurePublishingIntegrity() {
  if (hasValidatedPublishingData || typeof window !== "undefined") return;
  ...
  for (const reflection of reflections) {
    ...
    // Scripture is optional for a reflection. If supplied, it must still be valid.
    if (!isBlank(reflection.scripture) && !scriptureReferencePattern.test(reflection.scripture)) {
      errors.push(`Reflection "${reflection.contentSlug}" has malformed scripture "${reflection.scripture}".`);
    }
  ...
  for (const scripture of scriptureReferences) {
    ...
    if (!scriptureReferencePattern.test(scripture.reference)) errors.push(`Scripture "${scripture.slug}" has malformed reference "${scripture.reference}".`);
  ...
  for (const study of studies) {
    if (!studyDatePattern.test(study.date)) errors.push(`Study entry has malformed date "${study.date}".`);
  ...
  if (errors.length) throw new Error(`Publishing data validation failed:\n- ${errors.join("\n- ")}`);
}
