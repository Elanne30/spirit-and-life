...
export type RichTextDocument = {
  ...
  content: { type: "doc"; content: RichTextNode[] };
};
export type RichTextNode = {
  type: "heading" | "paragraph" | "bulletList" | "orderedList" | "listItem" | "text";
...
export type RichTextMark = {
  type: "bold" | "italic" | "underline" | "link" | "textStyle";
  attrs?: Record<string, unknown>;
};
...
function validLink(value: unknown) {
  ...
  try {
    ...
    return allowedSchemes.has(url.protocol) ? href : null;
  } catch {
    return null;
  }
}
...
function normalizeBlockAttrs(value: unknown, heading = false) {
  ...
  return heading ? { level: attrs.level === 3 ? 3 : 2, textAlign, indent } : { textAlign, indent };
}
...
export function legacySectionsToRichText(sections: LegacySection[] | undefined): RichTextDocument {
  ...
  for (const section of sections ?? []) {
    if (section.heading.trim()) content.push({ type: "heading", attrs: { level: 2, textAlign: "left", indent: 0 }, content: [{ type: "text", text: section.heading }] });
    for (const paragraph of section.paragraphs ?? []) if (paragraph.trim()) content.push({ type: "paragraph", attrs: { textAlign: "left", indent: 0 }, content: [{ type: "text", text: paragraph }] });
  }
  return { format: RICH_TEXT_FORMAT, version: RICH_TEXT_VERSION, content: { type: "doc", content: content.length ? content : [{ type: "paragraph" }] } };
}
