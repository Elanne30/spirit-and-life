...
function normalizeMarks(value: unknown): RichTextMark[] | undefined {
  ...
  for (const raw of value) {
    ...
    if (raw.type === "textStyle" && !seen.has("textStyle")) { const attrs = isRecord(raw.attrs) ? raw.attrs : {}; const fontFamily = fontFamilies.includes(attrs.fontFamily as FontFamily) ? attrs.fontFamily as FontFamily : undefined; const fontSize = fontSizes.includes(attrs.fontSize as FontSize) ? attrs.fontSize as FontSize : undefined; if (fontFamily || fontSize) marks.push({ type: "textStyle", attrs: { ...(fontFamily ? { fontFamily } : {}), ...(fontSize ? { fontSize } : {}) } }); seen.add("textStyle"); }
  }
  return marks.length ? marks : undefined;
}
function normalizeBlockAttrs(value: unknown, heading = false) { const attrs = isRecord(value) ? value : {}; const textAlign = textAlignments.includes(attrs.textAlign as TextAlignment) ? attrs.textAlign as TextAlignment : "left"; const indentValue = Number(attrs.indent); const indent = Number.isInteger(indentValue) ? Math.max(0, Math.min(maxIndent, indentValue)) : 0; return heading ? { level: attrs.level === 3 ? 3 : 2, textAlign, indent } : { textAlign, indent }; }
...
function isSupportedMarks(value: unknown) {
  ...
  return value.every((mark) => { if (!isRecord(mark) || typeof mark.type !== "string") return false; if (["bold", "italic", "underline"].includes(mark.type)) return true; if (mark.type === "link") return validLink(isRecord(mark.attrs) ? mark.attrs.href : undefined) !== null; if (mark.type === "textStyle") { const attrs = isRecord(mark.attrs) ? mark.attrs : {}; return (attrs.fontFamily === undefined || fontFamilies.includes(attrs.fontFamily as FontFamily)) && (attrs.fontSize === undefined || fontSizes.includes(attrs.fontSize as FontSize)); } return false; });
}
...
export function normalizeRichTextDocument(value: unknown): RichTextDocument | null { if (!isRecord(value) || value.format !== RICH_TEXT_FORMAT || value.version !== RICH_TEXT_VERSION || !isRecord(value.content) || value.content.type !== "doc") return null; if (!isSupportedNodes(value.content.content, "block")) return null; const content = normalizeNodes(value.content.content); return { format: RICH_TEXT_FORMAT, version: RICH_TEXT_VERSION, content: { type: "doc", content: content.length ? content : [{ type: "paragraph" }] } }; }
export function legacySectionsToRichText(sections: LegacySection[] | undefined): RichTextDocument { const content: RichTextNode[] = []; for (const section of sections ?? []) { if (section.heading.trim()) content.push({ type: "heading", attrs: { level: 2, textAlign: "left", indent: 0 }, content: [{ type: "text", text: section.heading }] }); for (const paragraph of section.paragraphs ?? []) if (paragraph.trim()) content.push({ type: "paragraph", attrs: { textAlign: "left", indent: 0 }, content: [{ type: "text", text: paragraph }] }); } return { format: RICH_TEXT_FORMAT, version: RICH_TEXT_VERSION, content: { type: "doc", content: content.length ? content : [{ type: "paragraph" }] } }; }
function nodeText(node: RichTextNode): string { if (node.type === "text") return node.text ?? ""; return (node.content ?? []).map(nodeText).join(""); }
export function richTextToLegacySections(document: RichTextDocument): LegacySection[] { const sections: LegacySection[] = []; let current: LegacySection | null = null; const addParagraph = (text: string) => { const value = text.trim(); if (!value) return; if (!current) { current = { heading: "", paragraphs: [] }; sections.push(current); } current.paragraphs.push(value); }; const walk = (nodes: RichTextNode[]) => nodes.forEach((node) => { if (node.type === "heading") { current = { heading: nodeText(node).trim(), paragraphs: [] }; sections.push(current); } else if (node.type === "paragraph") addParagraph(nodeText(node)); else if (node.type === "bulletList" || node.type === "orderedList" || node.type === "listItem") walk(node.content ?? []); }); walk(document.content.content); return sections.filter((section) => section.heading || section.paragraphs.length); }
