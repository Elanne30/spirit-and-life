...
function firstUnsupportedString(value: string, field: string): UnsupportedText | null {
  for (let index = 0; index < value.length; index += 1) {
    ...
    // PostgreSQL text/jsonb cannot store a NUL character.
    if (code === 0) return { field, codePoint: "U+0000" };

    // JavaScript can represent an unpaired UTF-16 surrogate even though it is
    // not a valid Unicode scalar value. PostgreSQL jsonb rejects malformed
    // surrogate escapes, so report the exact field instead of changing text.
