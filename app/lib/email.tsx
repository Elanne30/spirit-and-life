    ...
    <table role="presentation" cellpadding="0" cellspacing="0" width="100%" style="background-color:#f7f2ea;padding:32px 16px;">
      <tr><td align="center">
        <table role="presentation" cellpadding="0" cellspacing="0" width="100%" style="max-width:640px;background-color:#fffaf3;border:1px solid #e2d8c8;border-radius:20px;overflow:hidden;">
          <tr><td style="padding:40px 40px 24px;"><p style="margin:0;color:#8b6b43;font-size:12px;letter-spacing:0.18em;text-transform:uppercase;">Spirit &amp; Life</p><h1 style="margin:14px 0 0;font-size:34px;line-height:1.1;">${escapeHtml(props.headline)}</h1></td></tr>
          <tr><td style="padding:0 40px 8px;font-size:18px;line-height:1.7;">${bodyHtml}</td></tr>
          ${ctaHtml}
          ${footerHtml ? `<tr><td style="padding:12px 40px 4px;">${footerHtml}</td></tr>` : ""}
          <tr><td style="padding:16px 40px 40px;font-size:13px;line-height:1.6;color:#6f655b;"><p style="margin:0;">If you no longer want these emails, you can <a href="${escapeHtml(props.unsubscribeHref)}" style="color:#8b6b43;">unsubscribe here</a>.</p></td></tr>
        </table>
      </td></tr>
    </table>
  </body>
...
export function renderPlainTextNewsletter(params: { headline: string; paragraphs: string[]; ctaLabel?: string; ctaHref?: string; unsubscribeHref: string }) {
  const parts = [params.headline, "", ...params.paragraphs.map((paragraph) => toText(paragraph))];
  if (params.ctaLabel && params.ctaHref) parts.push("", `${params.ctaLabel}: ${params.ctaHref}`);
  parts.push("", `Unsubscribe: ${params.unsubscribeHref}`);
  return parts.join("\n");
}

async function sendViaResend(payload: EmailPayload, fromEmail: string, apiKey: string): Promise<EmailDeliveryResult> {
  try {
    const response = await fetch(RESEND_EMAIL_ENDPOINT, {
      method: "POST",
      headers: { accept: "application/json", authorization: `Bearer ${apiKey}`, "content-type": "application/json" },
      body: JSON.stringify({
        from: `${NEWSLETTER_SENDER_NAME} <${fromEmail}>`,
        to: [payload.to],
    ...
    if (!response.ok) {
      console.error(`[newsletter-email] Resend request failed with status ${response.status}.`);
      return { ok: false, error: response.status === 400 || response.status === 401 || response.status === 403 ? "Email delivery is not configured." : "Email delivery failed." };
    }
    const result = (await response.json()) as { id?: string };
    return { ok: true, id: result.id ?? crypto.randomUUID() };
  } catch {
    console.error("[newsletter-email] Resend transport error.");
...
async function sendViaBrevo(payload: EmailPayload, fromEmail: string, apiKey: string): Promise<EmailDeliveryResult> {
  try {
    const response = await fetch(BREVO_TRANSACTIONAL_EMAIL_ENDPOINT, {
      ...
      body: JSON.stringify({
        sender: { name: NEWSLETTER_SENDER_NAME, email: fromEmail },
        to: [{ email: payload.to }],
        subject: payload.subject,
    ...
    if (!response.ok) {
      console.error(`[newsletter-email] Brevo request failed with status ${response.status}.`);
      return { ok: false, error: response.status === 400 || response.status === 401 || response.status === 403 ? "Email delivery is not configured." : "Email delivery failed." };
    }
