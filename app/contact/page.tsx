...
export default function ContactPage() {
  ...
  return (
    <main className="contact-main">
      ...
      <style>{`
        .contact-main { background: var(--background); }
        .contact-main .contact-hero { padding-block: clamp(4.5rem, 8vw, 7rem) clamp(3.5rem, 6vw, 5rem); text-align: center; }
        .contact-main .contact-hero h1 { margin: .35rem 0 1rem; font-size: clamp(3.4rem, 7vw, 6rem); line-height: .9; letter-spacing: -.035em; }
        .contact-main .contact-hero > p:last-child { max-width: 43rem; margin-inline: auto; color: var(--muted); line-height: 1.75; }
        .contact-main .contact-layout { width: min(100% - 3rem, 78rem); margin-inline: auto; display: grid; grid-template-columns: minmax(0, 1.08fr) minmax(19rem, .92fr); gap: clamp(1.25rem, 3vw, 2rem); padding-bottom: clamp(5rem, 9vw, 8rem); }
        .contact-main .contact-form { padding: clamp(1.5rem, 3.5vw, 2.75rem); border: 1px solid var(--line); border-radius: .25rem; background: var(--surface); box-shadow: 0 1.2rem 3rem var(--shadow); }
        ...
        .contact-main .contact-form label { display: block; margin: 1rem 0 .45rem; font-size: .78rem; font-weight: 700; letter-spacing: .04em; text-transform: uppercase; }
        .contact-main .contact-form input:not([type="hidden"]), .contact-main .contact-form textarea { width: 100%; border: 1px solid var(--line); border-radius: .15rem; background: var(--background); color: var(--foreground); padding: .85rem .9rem; outline: none; transition: border-color 180ms ease, box-shadow 180ms ease; }
        ...
        .contact-main .contact-card { padding: 1.35rem 1.45rem; border: 1px solid var(--line); border-radius: .25rem; background: var(--surface); box-shadow: 0 .8rem 2rem var(--shadow); }
        ...
        .contact-main .contact-note { background: color-mix(in srgb, var(--surface) 72%, var(--accent)); }
        @media (max-width: 800px) { .contact-main .contact-hero { text-align: left; } .contact-main .contact-hero > p:last-child { margin-inline: 0; } .contact-main .contact-layout { width: min(100% - 1.5rem, 48rem); grid-template-columns: 1fr; } }
